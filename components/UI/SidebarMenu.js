			...
			<Link href="/">
				<a>
					<div className="flex items-center py-2 pr-8 bg-gray-200 justify-between">
						...
						<div className="bg-pink-300 text-white rounded h-6 w-6 items-center flex justify-center"
			...
			<div className="flex flex-col py-4">
				{menus.map((link) => {
					return <div className="py-2 mb-0 mt-1 hover:bg-pink-300" key={link.id} data-testid={link.id}>
						<Link href={link.link}>
							<a className="flex justify-end pr-8" onClick={props.onClose}>{link.title} </a>
						</Link>
					</div>;
				})}
			</div>
