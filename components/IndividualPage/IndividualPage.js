...
const IndividualPageProduct = ({title, body, img, id}) => {

	const prices = Math.floor(Math.random() * 100);

	return (<div className="p-1  bg-slate-100 rounded-md shadow-md" data-testid='individual-product'>
			<div className="mb-4 h-48" data-testid={title?.slice(0,4)}>
				...
				<p className="text-justify" data-testid={body?.slice(1,4)}>{body}</p>
			</div>
			<div className="py-2 md:py-6 px-1 my-2 mt-6 flex flex-col lg:flex-row justify-between" data-testid={body?.slice(7,15)}>
